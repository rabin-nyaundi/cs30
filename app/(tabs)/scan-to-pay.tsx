...
// import QRCode from 'react-native-qrcode-svg'; // Uncomment if installed
...
export default function ScanToPayScreen() {
  ...
  const handlePay = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      router.push({ pathname: '/receipt', params: { selected, name, license, payment: 'scan' } });
    }, 2000); // Simulate payment
  };

  // const qrValue = JSON.stringify({ name, license, totalFine });
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <View style={styles.qrPlaceholder}>
        <ThemedText type="default">[QR Code Here]</ThemedText>
      </View>
      <TouchableOpacity style={styles.button} onPress={handlePay} disabled={loading}>
        {loading ? <ActivityIndicator color="#fff" /> : <ThemedText type="default" style={{ color: '#fff' }}>Simulate Scan to Pay</ThemedText>}
      </TouchableOpacity>
    </ThemedView>
  );
}
